...
export default function AdminProducts() {
  ...
  const handleSave = () => {
    ...
    const images = [imgInput || 'https://images.unsplash.com/photo-1510557880182-3d4d3cba35a5?w=800&q=80'];
    if (editing) {
      dispatch(updateProduct({ ...form, id: editing.id, createdAt: editing.createdAt, tags, images }));
      toast.success('Product updated');
    } else {
      dispatch(addProduct({ ...form, id: `p-${Date.now()}`, createdAt: new Date().toISOString(), tags, images }));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display font-bold text-2xl text-white">Products</h2>
                ...
                <tr key={product.id} className="hover:bg-surface-hover transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {product.originalPrice && (
                      <span className="text-xs text-zinc-600 line-through ml-1">${product.originalPrice}</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`font-mono text-sm ${product.stock <= 10 ? 'text-red-400' : product.stock <= 30 ? 'text-yellow-400' : 'text-green-400'}`}>
                      {product.stock}
                    </span>
                  </td>
                  <td className="px-4 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-surface-border">
                ...
                <button onClick={() => setModalOpen(false)} className="p-2 rounded-lg text-zinc-400 hover:text-white hover:bg-surface-hover">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-5 space-y-4">
                {/* Image URL */}
                <div>
                  <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Product Image URL</label>
                  <div className="flex gap-2">
                    ...
                    />
                    {imgInput && (
                      <img src={imgInput} alt="" className="w-10 h-10 rounded-lg object-cover border border-surface-border" onError={e => (e.currentTarget.style.display = 'none')} />
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Product Name *</label>
                    <input type="text" value={form.name} onChange={set('name')} className="input" placeholder="iPhone 16 Pro" />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Brand *</label>
                    <input type="text" value={form.brand} onChange={set('brand')} className="input" placeholder="Apple" />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Price *</label>
                    <input type="number" value={form.price} onChange={set('price')} className="input" min={0} />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Original Price</label>
                    <input type="number" value={form.originalPrice || ''} onChange={set('originalPrice')} className="input" min={0} placeholder="Optional" />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Stock</label>
                    <input type="number" value={form.stock} onChange={set('stock')} className="input" min={0} />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Rating</label>
                    <input type="number" value={form.rating} onChange={set('rating')} className="input" min={0} max={5} step={0.1} />
                  </div>
                </div>

                <div>
                  <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Short Description</label>
                  <input type="text" value={form.shortDescription} onChange={set('shortDescription')} className="input" placeholder="Brief tagline..." />
                </div>
                ...
                <div className="flex gap-4">
                  {(['featured', 'trending', 'isNew'] as const).map(flag => (
                    <label key={flag} className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-zinc-400 capitalize">{flag === 'isNew' ? 'New' : flag}</span>
                    </label>
                  ))}
                </div>
              </div>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="card p-6 w-full max-w-sm animate-scale-in">
              <h3 className="font-display font-semibold text-white text-lg mb-2">Delete Product?</h3>
